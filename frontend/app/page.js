    ...
    <div className="flex flex-col min-h-screen">
      ...
      <section className="py-20 px-4 max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { title: "Banarasi Silk", img: "https://images.unsplash.com/photo-1583391733958-692b1baecd11?q=80&w=800" },
            { title: "Kanchipuram",   img: "https://images.unsplash.com/photo-1623091410901-00e2d268901e?q=80&w=800" },
            { title: "Wedding Wear",  img: "https://images.unsplash.com/photo-1595777457583-95e059d581b8?q=80&w=800" },
          ].map((cat, i) => (
            <div key={i} className="group relative h-96 rounded-2xl overflow-hidden cursor-pointer">
              ...
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-end pb-8">
                ...
                <span className="text-white/80 uppercase tracking-widest text-xs opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-500">Explore</span>
              </div>
            </div>
          ))}
        </div>
            ...
            {[1,2,3,4].map((item) => (
              <div key={item} className="bg-card rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow duration-300 border border-border group">
                <div className="relative aspect-[3/4] overflow-hidden">
                  <div className="absolute top-3 left-3 z-10 bg-white/90 dark:bg-black/50 backdrop-blur block px-2 py-1 text-xs font-bold uppercase tracking-wider rounded-md text-primary-600">Bestseller</div>
                  <img src={`https://images.unsplash.com/photo-1610030469983-98e550d615ef?q=80&w=400&auto=format&fit=crop&sig=${item}`}
                    alt="Saree" className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-500" />
                ...
                <div className="p-5">
                  <h3 className="font-semibold text-lg hover:text-primary-600 transition-colors line-clamp-1">Royal Kanchipuram Silk Saree</h3>
