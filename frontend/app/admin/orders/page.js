...
export default function OrdersPage() {
  ...
  useEffect(() => {
    const afterPrint = () => {
      // Optional: setOrderToPrint(null) if you want to hide the invoice from DOM after printing
      // But we can keep it hidden via CSS screen media query anyway.
    };
  ...
  return (
    <>
      ...
      <div className="space-y-6">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="p-2 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-all border border-transparent hover:border-primary-100 ml-auto"
                          title="View Details"
                        >
                          <Eye size={18} />
                        </button>
                        ...
                        {order.status !== 'pending' && order.status !== 'cancelled' && (
                          <button
                            ...
                            className="p-2 text-gray-400 hover:text-gray-800 hover:bg-gray-100 rounded-lg transition-all border border-transparent hover:border-gray-300"
                            title="Print Invoice"
                          >
                            <Printer size={18} />
                          </button>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="bg-gray-50 rounded-2xl p-4 space-y-2">
                      ...
                      {selectedOrder.shippingAddress?.phone && (
                        <p className="text-xs text-blue-600 font-bold">Shipping Phone: {selectedOrder.shippingAddress.phone}</p>
                      )}
                    </div>
                  </div>
        ...
        {orderToPrint && (
          <div id="printable-invoice" className="hidden print:block w-full text-black font-sans bg-white p-8">
            {/* Header */}
            <div className="flex justify-between items-start border-b-2 border-black pb-4 mb-6">
              <div className="flex items-center gap-2">
                <div className="w-16 h-16 flex items-center justify-center">
                  <img src="/ruva_logo_t.png" alt="Ruva Logo" className="w-full h-full object-contain" />
                </div>
                {/* <div>
                  <h1 className="text-3xl font-black tracking-tight uppercase">Ruva</h1>
                  <p className="text-xs font-bold text-gray-600 tracking-widest">E-COMMERCE</p>
                </div> */}
              </div>
              <div className="text-right">
                <h2 className="text-xl font-bold uppercase tracking-wider mb-1">Tax Invoice / Bill of Supply</h2>
            ...
            <div className="flex justify-between mb-8 text-sm">
              <div className="w-1/2 pr-4 border-r border-gray-300">
                <h3 className="font-bold text-gray-500 uppercase tracking-wider text-xs mb-2">Sold By</h3>
                <p className="font-bold">Ruva Handlooms</p>
                <p>Shop No 15, Atal Bihari Vajpayee, Opposite Bata Showroom,</p>
                <p>M.G. Road, Chikmagalur- 577101</p>
                {/* <p className="mt-1"><strong>GSTIN:</strong> 29AAACR1234A1Z5</p> */}
              </div>
              <div className="w-1/2 pl-4">
                <div className="flex justify-between mb-2">
                  <span className="font-bold text-gray-500 uppercase tracking-wider text-xs">Order ID:</span>
                  <span className="font-bold">{orderToPrint._id}</span>
                </div>
                <div className="flex justify-between mb-2">
                  <span className="font-bold text-gray-500 uppercase tracking-wider text-xs">Order Date:</span>
                  <span>{new Date(orderToPrint.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                </div>
                <div className="flex justify-between mb-2">
                  <span className="font-bold text-gray-500 uppercase tracking-wider text-xs">Invoice Date:</span>
                  <span>{new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                </div>
                <div className="flex justify-between">
                  <span className="font-bold text-gray-500 uppercase tracking-wider text-xs">Payment Mode:</span>
                  <span className="uppercase">{orderToPrint.paymentMethod}</span>
                </div>
              </div>
            </div>

            {/* Addresses */}
            <div className="flex justify-between mb-8 border border-gray-300">
              <div className="w-1/2 p-4 border-r border-gray-300">
                <h3 className="font-bold text-gray-500 uppercase tracking-wider text-xs mb-2">Billing Address</h3>
                <p className="font-bold text-base">{orderToPrint.user?.name || 'Guest User'}</p>
                <p className="mt-1">
                  {orderToPrint.shippingAddress?.street}<br />
                  {orderToPrint.shippingAddress?.city}, {orderToPrint.shippingAddress?.state}<br />
                  {orderToPrint.shippingAddress?.pincode}
                </p>
                <p className="mt-2"><strong>Phone:</strong> {orderToPrint.user?.phone || 'N/A'}</p>
              </div>
              <div className="w-1/2 p-4">
                <h3 className="font-bold text-gray-500 uppercase tracking-wider text-xs mb-2">Shipping Address</h3>
                <p className="font-bold text-base">{orderToPrint.user?.name || 'Guest User'}</p>
                <p className="mt-1">
                  {orderToPrint.shippingAddress?.street}<br />
                  {orderToPrint.shippingAddress?.city}, {orderToPrint.shippingAddress?.state}<br />
                ...
                <p className="mt-2"><strong>Phone:</strong> {orderToPrint.shippingAddress?.phone || orderToPrint.user?.phone || 'N/A'}</p>
              </div>
            </div>
            ...
            <table className="w-full text-left border-collapse border border-gray-300 mb-8 text-sm">
              <thead>
                <tr className="bg-gray-100">
                  <th className="border border-gray-300 p-2 font-bold uppercase text-xs w-10 text-center">No.</th>
                  <th className="border border-gray-300 p-2 font-bold uppercase text-xs">Product Description</th>
                  <th className="border border-gray-300 p-2 font-bold uppercase text-xs text-center w-20">Qty</th>
                  <th className="border border-gray-300 p-2 font-bold uppercase text-xs text-right w-28">Unit Price</th>
                  <th className="border border-gray-300 p-2 font-bold uppercase text-xs text-right w-28">Total</th>
                </tr>
              </thead>
              <tbody>
            ...
            <div className="flex justify-between items-end mt-16 pt-8 border-t border-gray-300">
              <div className="text-xs text-gray-500 w-2/3">
                <p className="mb-1"><strong>Declaration:</strong></p>
