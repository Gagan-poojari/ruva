...
export default function AdminLayout({ children }) {
  ...
  if (!isAdmin) return null; // Or a loader
    ...
    <div className="min-h-screen bg-[#f8fafc] flex">
      ...
      >
        <div className="p-6 border-b border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-primary-600 flex items-center justify-center font-bold text-white">R</div>
            <span className="text-xl font-bold tracking-tight">Ruva <span className="text-primary-500">Admin</span></span>
          </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-primary-500 shadow-[0_0_8px_rgba(var(--primary-color),0.5)]" />}
              </Link>
            );
          })}
        </nav>
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 z-40">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 lg:ml-0 ml-4">
            <span>Admin</span>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-gray-200">
              <div className="text-right hidden sm:block">
                <p className="text-sm font-bold text-gray-900 leading-none">{adminData?.name || 'Admin User'}</p>
                <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-1">Super Admin</p>
              </div>
              <div className="w-10 h-10 rounded-full bg-slate-100 border border-gray-200 flex items-center justify-center text-primary-600">
                <User size={20} />
              </div>
            </div>
          </div>
